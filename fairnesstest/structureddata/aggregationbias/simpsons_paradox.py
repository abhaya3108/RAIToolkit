...
class SimpsonsParadox:
    ...
    def get_simpsons_pairs(self):
        ...
        for pair in candidate_pairs:
            ...
            if (np.sign(coef) != np.sign(coef_sign_sum) and
                ...
                if self.output_plots:
                    ...
                    #display(simple_reg)
                    output.append(f'Independent Variable: {iv}')
                    simple_reg = simple_reg.reset_index(drop=True)
                    output.append(simple_reg.to_html())
                    self.plot_trendline(cv=None, iv=iv,
                                        predictions=iv+'_predictions')
                    fig = plt.gcf()
                    output.append(mpld3.fig_to_html(fig))
                    # Plot single-variable regressions
                    print('Conditioned on:', cv)
                    output.append(f'Conditioned on: {cv}')
                    multiple_reg = multiple_reg.reset_index(drop=True)
                    output.append(multiple_reg.to_html())
        ...
        if not self.quiet:
            if simpsons_pairs == []:
                results['msg'] = 'Congratulations! No Simpson’s Pairs were detected (e.g. Simpson’s Paradox was not detected for your dataset).'
                print('Congratulations! No Simpson’s Pairs were detected (e.g.'
                      ' Simpson’s Paradox was not detected for your dataset).')
                results['simpsons_pairs'] = "None"
            else:
                results['msg'] = f"{len(simpsons_pairs)} Simpson’s Pair(s) were detected in your dataset."
                print('%d Simpson’s Pair(s) were detected in your dataset.' %
                      len(simpsons_pairs))
                results['simpsons_pairs'] = pd.DataFrame(simpsons_pairs, columns=['Feature #1', 'Feature #2']).to_html()
